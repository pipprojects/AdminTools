...
do
 ...
 chown ${UUID}:${UGID} $acc
 chmod 600 $acc
 FOLDER="$(echo ${IMAPF}/${acc} | sed -e s@"/\{2,\}"@"/"@g)"
 echo "Info: Now subscribe to the folder ${FOLDER} in account $USERNAME"
 SMES="${acc}"
 PMFILE="rc.support-emails"
 a=$PWD
...
echo '#' >> $OUTFILE
echo "# Send files to folder" >> $OUTFILE
echo "# $0" >> $OUTFILE
echo '#' >> $OUTFILE
echo ":0" >> $OUTFILE
echo "* ^X-Tag-Header: ${acc}$" >> $OUTFILE
echo "${IMAPF}/${acc}" >> $OUTFILE
echo "#" >> $OUTFILE
echo "$TAGEND" >> $OUTFILE
#
case $OVERW in
...
#
USAGE="$PROGNAME -u Username -f IMAPFolder [-q] [-v] [-h] [-H] [-V]"
HELPINFO[1]="-u Username"
HELPINFO[2]="-f IMAP folder"
...
then
 ...
 then
  ...
  chmod 755 $IMAPF
   _ADM__Log 3 "Creating $IMAPF directory" 1 $SENDEML 1
 else
  if [ ! -d "${IMAPF}" ]
  then
